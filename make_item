...
meta_re = re.compile(r'\s*(Lines|Language|Language code|Author|Date|' +\
...
def read_headers(tslines):
  ...
  if not headers['language']:
    print(err % 'language name',file=sys.stderr)
    sys.exit(1)
  
  if not headers['language code']:
  ...
  if not headers['lines']:
    print(err % 'a list of line names',file=sys.stderr)
    sys.exit(1)

  for l in headers['lines'].split():
    if not l in blessed_line_names:
      ...
      print(str(headers['lines']),file=sys.stderr)
      sys.exit(1)
    elif l.endswith('-seg'):
